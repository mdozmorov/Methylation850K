...
```{r settings}
source("AgeTest/NORMALIZATION.R")
runImport = FALSE
# Lucas's path
...
probeAnnotation21kdatMethUsed=read.csv("AgeTest/probeAnnotation21kdatMethUsed.csv")
probeAnnotation27k=read.csv("AgeTest/datMiniAnnotation27k.csv")
datClock=read.csv("AgeTest/AdditionalFile3.csv")
```
...
file.remove("AgeTest/LogFile.txt")
file.create("AgeTest/LogFile.txt")
DoNotProceed=FALSE
cat(paste( "The methylation data set contains", nSamples, "samples (e.g. arrays) and ", nProbes, " probes."),file="AgeTest/LogFile.txt")
...
Samples correspond to columns in that file ."), file="AgeTest/LogFile.txt",append=TRUE) }
if (nProbes==0) {DoNotProceed=TRUE; cat(paste( "\n ERROR: There must be a data input error since there seem to be zero
probes.\n Make sure that you input a comma delimited file (.csv file)\n that can be read using the R command read.csv.sql
CpGs correspond to rows.") , file="AgeTest/LogFile.txt",append=TRUE) }
...
the data and then resubmit them? In any event, I will proceed with the analysis."),file="AgeTest/LogFile.txt",append=TRUE) }
...
contains probe identifiers such as cg00000292. Instead it contains ", dat0[1:3,1] ),file="AgeTest/LogFile.txt",append=TRUE) }
...
identifiers such as cg00000292. Instead it contains ", dat0[1:3,1] ),file="AgeTest/LogFile.txt",append=TRUE) }
...
this makes sense.\n" ),file="AgeTest/LogFile.txt",append=TRUE) }
...
(or probe names):\n ", paste( missingProbes, sep="",collapse=", ")),file="AgeTest/LogFile.txt",append=TRUE) } 
}
```
...
normalizeData=TRUE
source("AgeTest/StepwiseAnalysis.R")
```
...
datout[,1][datout$Comment != ""], "\n Hint: Check the output file for more details."),file="AgeTest/LogFile.txt",append=TRUE) }
...
index = 1
tryCatch(
  for (ID in unique(sample.annotation$ID)) {
...
cor(sample.annotation$Age, sample.annotation$DNAmAge)
...
cor(sample.annotation$Age[groupAIndex], sample.annotation$DNAmAge[groupAIndex])
...
cor(sample.annotation$Age[groupBIndex], sample.annotation$DNAmAge[groupBIndex])
