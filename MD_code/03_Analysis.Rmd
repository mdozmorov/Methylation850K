...
fileNameOut1       = file.path(data.dir, "Enrichment.xlsx")
...
```{r}
mtx <- read_xlsx(fileNameIn1, sheet = "DEGs.coding")
...
for (gs_cat in m_df_gs_cat) {
  ...
  res.msigdf.em <- res.msigdf.em[res.msigdf.em$p.adjust < p_adj_cutoff, ]
  em2 <- GSEA(geneList, TERM2GENE = m_t2g)
  res.msigdf.em2 <-em2@result
  res.msigdf.em2 <- res.msigdf.em2[res.msigdf.em2$p.adjust < p_adj_cutoff, ]
